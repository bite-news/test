...
from db.crud import insert_article, update_article, get_article_list, search_article_list
...
async def receive_article_and_make_shortform(article_request_dto: ArticleRequestDto, session=Depends(get_session)):
    ...
    try:
        ...
        # ✅ 2️⃣ 썸네일 & 비디오 생성
        video_path, thumbnail_path = create_article(
            article_request_dto.title, article_request_dto.content)
        # local_thumbnail_path = "thumbnail.png"
...
@app.get("/api/video", status_code=200)
def get_video_list(page: int = 1, session=Depends(get_session)):
    """✅ 최신순으로 비디오 리스트 조회 (Infinite Scroll)"""
    try:
        return {"status": "success", "message": "✅ 기사 페이징 조회 성공!", "data": get_article_list(session, page)}
    except Exception as e:
...
@app.get("/api/video/search", status_code=200)
def search_video_list(keyword: str, session=Depends(get_session)):
    """ ✅ 검색어로 숏폼 검색 """
    try:
        return {"status": "success", "message": "✅ 기사 검색 성공!", "data": search_article_list(session, keyword)}
    except Exception as e:
        logger.error(f"❌ 비디오 검색 오류: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@app.get("/api/video/make", status_code=201)
def make_shortform():
    """✅ 숏폼 생성 요청 API"""
