...
def load_font():
    """폰트를 한 번만 로드하는 함수"""
    font_path = config.get('font_file', 'GmarketSansTTFBold.ttf')
...
def generate_image(scene_dialogue, scene_number, fallback_images=None):
    ...
    # 뉴스 보도 스타일 프롬프트
    dalle_prompt = config.get('image_prompt',
                              "현대적인 한국 뉴스 보도 스타일의 고품질, 사실적인 1024x1024 이미지를 생성하세요. "
        ...
        while True:
            try:
                current_font = ImageFont.truetype(
                    config.get('font_file', 'GmarketSansTTFBold.ttf'), size=font_size) if font != ImageFont.load_default() else font
...
def generate_fast_tts(text, output_filename, speed=None):
    """gTTS를 사용하여 TTS 생성 및 속도 조절"""
    # config에서 속도 값 가져오기
    if speed is None:
        speed_settings = config.get('speed_settings', {})
        speed = speed_settings.get('tts_speed', 1.25)
...
def create_video_clip(image_file, audio_file, output_file):
    ...
    audio_bitrate = ffmpeg_settings.get('audio_bitrate', '192k')
...
def create_article(content, title):
    ...
    thumbnail_path = None
    if scene_images:
        first_scene = next((img for id, img in scene_images if id == 1), None)
        if not first_scene and scene_images:
            first_scene = scene_images[0][1]

        if first_scene:
            thumbnail_path = os.path.join(OUTPUT_DIR, "thumbnail.png")
            try:
                if overlay_title_on_image(first_scene, overall_title, thumbnail_path):
    ...
    for scene_id, image_path in scene_images:
        ...
        # TTS 생성
        speed_settings = config.get('speed_settings', {})
        tts_speed = speed_settings.get('tts_speed', 1.25)

        audio_path = get_temp_filepath(f"scene_{scene_id}_fast.mp3")
        if not generate_fast_tts(dialogue, audio_path, speed=tts_speed):
            continue
